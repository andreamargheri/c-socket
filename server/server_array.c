...
//mutex for threads thread data (e.g., files list and username)
pthread_mutex_t thread_mutex[NUM_THREAD];
//mutex for share blocking
pthread_mutex_t share_mutex[NUM_THREAD];
...
int main(int argc,char * argv[]){
	...
	//int r = sem_init(&sem_n, 0, NUM_THREAD);	
	//printf("%d\n",r);
	...
	for (t =  0; t< NUM_THREAD; t++){
		...
		pthread_mutex_init(&thread_mutex[t],NULL);
		//mutex for the share thread 
		pthread_mutex_init(&share_mutex[t],NULL);
		//block mutex to intially halt the thread
		pthread_mutex_lock(&t_mutex[t]);
		//block mutex to hatl the thread when the share command is executed
		pthread_mutex_lock(&share_mutex[t]);		
...
void* sharing_thread(void* thread_data){
	...
	while(1){
		...
		if(register_user(this_data) == -1) {
			printf("THREAD %d: Errore impostazione username\n", th_id);
			close_client(this_data);
			continue;
		}
...
int register_user(sharingTr_data* this_data){ 
	...
	while(1){	
		...
		for (i = 0; i<NUM_THREAD; i++){
			//take the lock on the username list		
			pthread_mutex_lock(&thread_mutex[i]);
			// return 0 if equal
			if (strcmp(thread_data[i].client_user,user_name)==0){
				flag = 1;
				//release the lock on the username list
				//also here otherwise if the username is equal the lock is not unlocked		
...
int register_file(sharingTr_data* this_data){
	...
	ret = recv(this_data->sk, mess, BUF_SIZE, MSG_WAITALL); 
	if( (ret == -1) || (ret<BUF_SIZE) ) {
		printf("THREAD %d: Errore ricezione numero file\n", th_id);
		ret = -1;
		return -1;
	}
...
int share(sharingTr_data* this_data){
	...
	*/
	...
	//this mutex halts the thread untill a sharing action is completed (i.e., the get command unlock the mutex)
	pthread_mutex_lock(&share_mutex[th_id]);
		
	//send message for stopping the share command in the file_sender process	
	strcpy(mess,"SHARE FINISHED");
	if(send(this_data->sk, mess, BUF_SIZE ,0) < BUF_SIZE){
		printf("THREAD %d: Errore Invio conferma SHARE\n",th_id);
		close_client(this_data);
		return 0;
	...
	//set the sender status to NO_SHARED
	this_data->state_file = NO_SHARED;
	printf("THREAD %d: L'utente %s uscito dall modalita' SHARE\n",this_data->th_id,this_data->client_user);	
...
int file_list(sharingTr_data* this_data){
	char mess[BUF_SIZE];
	int th_id = this_data->th_id;
	int n_file = 0, i,dim=0,y;
	fl_list* tmp;
	//fl_list* tmp1;

	//estimating the maximum size of the array list
	for (i=0; i<NUM_THREAD; i++){
		if (thread_data[i].n_file > 0){
			dim = dim + thread_data[i].n_file;	
		}
	}
	//declaring the array of dimesion dim of char of dimension BUF_SIZE
	char l_array[dim][BUF_SIZE];
	...
	for (i=0; i<NUM_THREAD; i++){
		if (thread_data[i].n_file > 0){
			...
			while(tmp != NULL){
				//varible for checking if the file is already in the list
				int flag = 0;
				//check the list looking for the file_name file
				for (y=0; y<n_file; y++){
					if (strcmp(l_array[y],tmp->file_name)==0){
						flag = 1;
						break;
					}
				}
				if (flag == 0){
					//copy file name if it is not already in
					strcpy(l_array[n_file],tmp->file_name);				
					n_file = n_file + 1;
				}
	...
	if (n_file >0){
		//we send n_file files
		for (i=0;i<n_file;i++){
			strcpy(mess,l_array[i]);
			//send file name
...
int file_get(sharingTr_data* this_data){
	...
	if( (ret == -1) || (ret<BUF_SIZE) ) {
		printf("THREAD %d: Errore ricezione file name\n", th_id);
		return 0;
	}
	...
	if (status == 1){
		...
		if(send(sk_send, resp, BUF_SIZE ,0) < BUF_SIZE){
			...
			//unlock the sender thread 
			pthread_mutex_unlock(&share_mutex[i]);			
			return 0;
		}
		...
		if( (ret == -1) || (ret<BUF_SIZE) ) {
			printf("THREAD %d: Errore ricezione dimensione file da sender\n", th_id);
			//send err_get[2] to the receiver
			strcpy(resp,err_get[2]);			
			if(send(this_data->sk, resp, BUF_SIZE ,0) < BUF_SIZE){
				printf("THREAD %d: Errore invio messaggio receiver\n",th_id);
		...
		*/
		...
		if (status == 2){
			...
			printf("Sender non disponibile\n");
		}else {
			//file not found
			strcpy(resp,err_get[1]);			
			printf("File non trovato nel sistema\n");
		}
		if(send(this_data->sk, resp, BUF_SIZE ,0) < BUF_SIZE){
			printf("THREAD %d: Errore invio messaggio receiver\n",th_id);
			//reabilitate the SHARED state of the sender
